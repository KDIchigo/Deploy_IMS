...
export const SettingTable = ({
  ...
  const data = [];
  for (let index = 0; index < subjectSettings.data.length; index++) {
    const issue_setting = subjectSettings.data[index];
    let i = index + 1;
    data.push({
      ...
      issue_group: issueGroup(parseInt(issue_setting.issue_group, 10)),
    ...
    <div className="d-flex flex-column flexGrow_1">
      ...
      >
        ...
        <BasePagination
          ...
          totalRecord={subjectSettings.totalRecord}
        />
      </ConfigProvider>
    </div>
  );
